...
impl Png {
    ...
    pub fn new (data: Vec<u8>) -> Self {
        ...
        if data.len() >= crate::constants::LENGTH_OF_SIGNATURE {
            let mut signature = vec![0; crate::constants::LENGTH_OF_SIGNATURE];
...
pub fn create_png_from_deflated_data(width: u32, height: u32, inflated_data: *mut InflatedData, out_put_file_path: &Path) -> Option<Png> {
    unsafe {
        ...
        buffer.extend_from_slice(&crate::constants::LENGTH_OF_IHDR_DATA);

        // 2.2 Add IHDR type (4 bytes)
        //buffer.extend_from_slice(b"IHDR"); // Or use your constants::PNG_IHDR_TYPE_SIGNATURE
        buffer.extend_from_slice(&crate::constants::PNG_IHDR_TYPE_SIGNATURE);
...
pub fn create_png_from_png_files<T> (block_file_names: &Vec<String>, width: u32, height: u32, channels: u8) -> Option<Png> {
    ...
    let mut png_file_color_type: u8 = 0;
    let mut png_file_bit_depth: u8 = 0;
            ...
            match file {
                ...
                Ok (mut f) => {
                    ...
                    for i in 0..png_file_height {
                        ...
                        //all_compressed_idat_data[i] = (*dat_without_filter_method_byte)[i];
                    }
